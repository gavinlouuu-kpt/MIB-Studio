...
std::string selectSaveDirectory(const std::string &configPath)
{
    ...
    else if (choice == 3)
    {
        ...
        {
            std::error_code ec;
            std::filesystem::remove_all(testPath, ec);
            if (ec)
            {
                std::cout << "Warning: failed to clear '" << testPath.string() << "' (" << ec.message() << "). Using suffixed directory." << std::endl;
            }
        }
    ...
    // Create the subdirectory (no-throw) and fallback to suffixed variants if needed
    {
        std::error_code ec;
        std::filesystem::create_directories(fullPath, ec);
        if (ec)
        {
            // Try a bounded number of suffixed alternatives
            bool created = false;
            const int maxAttempts = 100;
            int attempt = suffix; // continue from next suffix
            while (attempt < suffix + maxAttempts && !created)
            {
                std::filesystem::path candidate = outputDir / (saveDir + "_" + std::to_string(attempt));
                std::error_code ec2;
                std::filesystem::create_directories(candidate, ec2);
                if (!ec2)
                {
                    if (candidate != fullPath)
                    {
                        std::cout << "Using fallback save directory: " << candidate.string() << std::endl;
                    }
                    fullPath = candidate;
                    created = true;
                    break;
            ...
            if (!created)
            {
                std::cout << "Warning: could not create save directory after multiple attempts. Proceeding with path: " << fullPath.string() << std::endl;
            }
        }
    }
    ...
    std::cout << "Using save directory: " << absolutePath.string() << std::endl;
...
json readConfig(const std::string &filename)
{
    ...
    else
    {
        ...
        // Set defaults for any missing fields (in-memory only; do not write back here)
